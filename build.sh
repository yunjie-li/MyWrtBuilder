...
# 只构建 ext4-combined-efi 镜像的配置
CONFIG_OPTS="CONFIG_TARGET_ROOTFS_SQUASHFS=n CONFIG_TARGET_ROOTFS_EXT4FS=y CONFIG_TARGET_EXT4_JOURNAL=y CONFIG_GRUB_EFI_IMAGES=y CONFIG_GRUB_IMAGES=n CONFIG_TARGET_IMAGES_PAD=n CONFIG_VMDK_IMAGES=n CONFIG_VDI_IMAGES=n CONFIG_VHDX_IMAGES=n CONFIG_QCOW2_IMAGES=n"
...
make image PROFILE="$PROFILE" CONFIG_TARGET_KERNEL_PARTSIZE="$CONFIG_TARGET_KERNEL_PARTSIZE" CONFIG_TARGET_ROOTFS_PARTSIZE="$CONFIG_TARGET_ROOTFS_PARTSIZE" PACKAGES="$PACKAGES" FILES="$FILES" DISABLED_SERVICES="$DISABLED_SERVICES" $CONFIG_OPTS
